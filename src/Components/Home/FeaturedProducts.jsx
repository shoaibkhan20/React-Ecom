...
function FeaturedProducts() {
  ...
  return (
        <section className="text-gray-600 body-font pt-20">
          ...
          <div className="container px-5 sm:px-24 py-24 mx-auto flex justify-between items-start">

            <aside className="sticky top-20 left-3 w-[20%]"> {/* categories sidebar */}
              ...
              <ul>
                 <li className={`border-b mt-3 ${currentCategory=='all'?'category_active':''} rounded`}>
                     <button className="py-3 px-3 uppercase text-sm" onClick={()=>{setCategory('all');setLimit(6);}}>All</button>
                 </li>
                {
                    categores.map((item)=>{
                      return <li key={item} className={`border-b ${currentCategory==item?'category_active':''} rounded`}
                                onClick={()=>setCategory(item)} >
                               <button className="py-3 px-3 uppercase text-sm block"  >{item}</button>
                            </li>
                    })
                }
              </ul>
            ...
            <div className="flex flex-wrap -m-4 w-[80%]">
              {
                data.map((item , index)=>{
                  return ( 
                    <div key={item.id*index} className="box lg:w-1/3 md:w-1/2 p-2 w-full h-[100%] my-7 hover:shadow-md">
